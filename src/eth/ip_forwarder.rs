...
pub fn process_packet(tx: &mut Sender<Event>, state: &mut SharedState, eth: EthernetPacket, interface: &NetworkInterface) {
    ...
    let clients = state.get(|state| {
        ...
        Some((source_client.clone(), dest_client.clone(), next_hop_client.clone()))
    });
    ...
    log::debug!("forwarding packet from {:?} to {:?} via next hop {:?}", source_client, dest_client, next_hop_client);
...
fn find_next_hop_client<'a>(state: &'a State, source_client: &'a state::Client, dest_client: &'a state::Client) -> &'a Client  {
    if source_client == dest_client {
        log::debug!("source client is equal to dest client, looping back");
        return dest_client;
    }

    let source_index = state.clients.iter().position(|c| c == source_client).unwrap();
...
fn send_packet_to_next_hop(tx: &mut Sender<Event>, next_hop: Client, interface: &NetworkInterface, eth: EthernetPacket) {
    if interface.mac.is_none() {
        log::warn!("could not forward packet on interface {} as mac is not known", interface.name);
        return;
    }

    if next_hop.mac.is_none() {
        log::warn!("could not forward packet to next hop {} as mac is not known", next_hop.name);
        return;
    }
